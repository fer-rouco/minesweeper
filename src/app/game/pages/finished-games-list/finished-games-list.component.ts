...
export class FinishedGamesListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    let finishedGameList: Array<FinishedGameItemInterface> = this.boardService.getFinishedGameList();
    ...
    if (finishedGameList) {
      ...
      this.rowObjects = finishedGameList.map((finishedGameItem: FinishedGameItemInterface) => {
        const formatDate: (date: string) => string = (date: string) => {
          if (!date || date.trim().length === 0) {
            return '';
          }
          return dateTimeFormat.format(new Date(date)).replaceAll('/', '-').replace(',', '')
        }
        return {
